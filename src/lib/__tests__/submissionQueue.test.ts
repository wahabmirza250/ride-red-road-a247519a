...
describe("dispatch", () => {
  it("starts leased bills and clears their lease", async () => {
    ...
    expect(records.every((r) => r.status === "submitting")).toBe(true);
    expect(records.every((r) => r.submit_locked_until === null)).toBe(true);
  });

  it("paces the same passenger without burning an attempt", async () => {
    const records = [
      makeRecord("1", { status: "submitting", jobId: "j1", riderId: "riderA" }),
      makeRecord("2", { status: "submitting", jobId: "j2", riderId: "riderA" }),
    ...
    const parked = records.find((r) => r.id === "3")!;
