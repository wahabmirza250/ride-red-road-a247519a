  ...
  for (const t of tokens ?? []) {
    ...
    try {
      const res = await fetch(`https://fcm.googleapis.com/v1/projects/${auth.project}/messages:send`, {
        ...
        body:JSON.stringify({message:{token:t.token,notification:{title:'NEMT Solutions',body:'New activity in your company app. Open the app to view it.'},data:{url:payload.url??'/'},android:{priority:'high',ttl:'60s'}}}),
      });
      if (res.ok) sent++;
