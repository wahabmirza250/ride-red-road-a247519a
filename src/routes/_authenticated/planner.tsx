...
import { adminReassignDriver, adminListAssignableDrivers } from "@/lib/dispatchAdmin.functions";
import { rescheduleRide } from "@/lib/dispatchAdmin.functions";
...
function PlannerPage() {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      toast.error(e instanceof Error ? e.message : "Could not load planner");
      setRides([]);
    }
  }, [load, from, to]);
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <h1 className="font-display text-xl font-semibold">Planner</h1>
          <p className="text-sm text-muted-foreground">
            Assign drivers ahead of time and adjust future pickup windows.
          </p>
        </div>
      ...
      {rides === null ? (
        ...
        <p className="rounded-2xl border border-border bg-surface p-6 text-center text-sm text-muted-foreground">
          Nothing to plan in this window.
        </p>
      ) : (
