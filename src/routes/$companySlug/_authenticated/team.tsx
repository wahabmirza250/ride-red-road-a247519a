    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <AppCard
          ...
          title="Driver app"
          desc={`/${slug}/driver/signin`}
          icon={<Car className="h-5 w-5" />}
          tone="primary"
        />
        <AppCard
          href={`${origin}/${slug}/dispatch/signin`}
          title="Dispatch app"
          desc={`/${slug}/dispatch/signin`}
          icon={<Radio className="h-5 w-5" />}
          tone="sky"
        />
        <AppCard
          href={`${origin}/${slug}/billing/signin`}
          title="Billing app"
          desc={`/${slug}/billing/signin`}
...
function AppCard({
  ...
  tone: "amber" | "primary" | "emerald" | "sky";
}) {
  const toneMap = {
...
function BillingStaffCard() {
  ...
  const removeBiller = useServerFn(deleteBillingUser);
  const billers = useQuery({
    queryKey: ["billing_users"],
  ...
  async function remove(userId: string, name: string) {
    ...
    try {
      ...
      toast.success("Billing user removed");
      qc.invalidateQueries({ queryKey: ["billing_users"] });
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "Failed to remove billing user");
    } finally {
      setRemovingId(null);
    }
  }
    ...
    <div className="rounded-2xl border border-border bg-surface p-5">
      <div className="mb-4 flex items-center justify-between">
        ...
        <Dialog open={openNew} onOpenChange={setOpenNew}>
          <DialogTrigger asChild>
            <Button size="sm" className="rounded-full">
              <Plus className="mr-2 h-4 w-4" /> Create billing user
            </Button>
          </DialogTrigger>
          <NewBillingUserDialog onClose={() => setOpenNew(false)} />
        </Dialog>
      </div>
        ...
        <div className="space-y-2">
          {(billers.data ?? []).map((d) => (
            ...
            >
              <div className="min-w-0">
                <div className="text-sm font-medium">
                  {[d.first_name, d.last_name].filter(Boolean).join(" ") || "Billing user"}
                </div>
                <div className="truncate text-xs text-muted-foreground">{d.email}</div>
              </div>
              <div className="flex items-center gap-2">
...
function NewBillingUserDialog({ onClose }: { onClose: () => void }) {
  const qc = useQueryClient();
  const create = useServerFn(createBillingUser);
  const [form, setForm] = useState({
    email: "",
    password: "",
    first_name: "",
  ...
  async function submit() {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : "Failed to create billing user");
    } finally {
      setSubmitting(false);
    }
  }
