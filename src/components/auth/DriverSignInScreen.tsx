...
export function DriverSignInScreen({ companySlug }: { companySlug?: string }) {
  ...
  useEffect(() => {
    ...
    if (isDriver) window.location.replace(companySlug ? `/${companySlug}/driver` : "/driver");
  }, [loading, user, isDriver, companySlug, submitting]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      window.location.replace(slug ? `/${slug}/driver` : "/driver");
    } catch (err) {
      ...
      <div className="w-full max-w-md animate-rise-in">
        ...
        <div className="rounded-3xl border border-border/60 bg-surface/70 p-8 shadow-lift backdrop-blur-xl">
          ...
          <div className="mt-6 text-center text-xs text-muted-foreground">
            Passenger?{" "}
            <a
              href={companySlug ? `/${companySlug}/passenger` : "/passenger"}
