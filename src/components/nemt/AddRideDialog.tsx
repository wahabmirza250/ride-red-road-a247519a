import { localInputToISOString } from "@/lib/operationStatus";
...
import {
  dispatchCreatePassenger,
  dispatchListPassengers,
  dispatchScheduleRide,
} from "@/lib/dispatchSchedule.functions";
...
export function AddRideDialog({
  ...
  async function savePassenger() {
    ...
    try {
      const passenger = (await createPassenger({ data: newPassenger })) as Passenger & {
        reused?: boolean;
      };
      ...
      toast.success(
        passenger.reused ? "Existing passenger selected" : "Passenger added and selected",
      );
    } catch (e) {
    ...
    try {
      const result = await schedule({
        data: {
          ...
          scheduled_pickup_time: localInputToISOString(when),
          driver_id: driverId || null,
          notes,
        },
      });
      if (result.assignmentWarning) toast.warning(result.assignmentWarning);
      else toast.success("Ride scheduled");
      setOpen(false);
      setPassengerId("");
      setPickup("");
      setDropoff("");
        ...
        <DialogContent className="max-h-[90vh] max-w-lg overflow-y-auto">
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  variant="outline"
                  size="sm"
                  onClick={() => setAddingPassenger((v) => !v)}
                >
                  <UserPlus className="mr-1 h-4 w-4" />
                  {addingPassenger ? "Cancel" : "Add new passenger"}
                </Button>
              </div>
              {addingPassenger && (
                <div className="space-y-3 rounded-xl border border-primary/30 bg-primary/5 p-3">
                  <div className="grid grid-cols-2 gap-2">
                    <Input
                      ...
                      onChange={(e) =>
                        setNewPassenger({ ...newPassenger, full_name: e.target.value })
                      }
                    />
                    <Input
                      placeholder="Medicaid ID *"
                      value={newPassenger.medicaid_id}
                      onChange={(e) =>
                        setNewPassenger({
                          ...newPassenger,
                          medicaid_id: e.target.value.toUpperCase(),
                        })
                      }
                    />
                  </div>
                  ...
                  <AddressAutocomplete
                    ...
                    onChange={(v) => setNewPassenger({ ...newPassenger, address: v })}
                    onResolve={(p) => setNewPassenger({ ...newPassenger, address: p.address })}
                  />
                  <Textarea
                    rows={2}
                    placeholder="Passenger notes (optional)"
                  ...
                  >
                    {creatingPassenger && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Save and
                    select passenger
                  </Button>
                </div>
              )}
              ...
              <div className="max-h-40 overflow-auto rounded-xl border border-border">
                {filtered.slice(0, 25).map((p) => (
                  <button
                    ...
                    className={`flex w-full items-center justify-between border-b border-border px-3 py-2 text-left text-sm last:border-b-0 hover:bg-accent ${passengerId === p.id ? "bg-primary/10" : ""}`}
                  >
                    <span>{p.name}</span>
                ...
                {filtered.length === 0 && (
                  <div className="p-3 text-center text-xs text-muted-foreground">
                    No passengers match. Use “Add new passenger” above.
                  </div>
                )}
              </div>
            </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={submit} disabled={saving}>
              {saving && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Schedule ride
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
