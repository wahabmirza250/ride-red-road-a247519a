...
begin
  ...
  with records as (
    ...
    from driver_trip_drafts f left join drivers d on d.user_id=f.driver_id and d.company_id=company left join riders r on r.id=f.rider_id and r.company_id=company where f.company_id=company and f.status='in_progress'
    union all
  ...
  ), page as (select * from filtered order by at desc nulls last,source,id limit 50 offset p_page*50)
  ...
  ) order by at desc nulls last,source,id),'[]'::jsonb)) into result from page;
  return result;
end $$;
...
begin
  ...
  select d.* into driver from drivers d where d.company_id=company and d.merged_into is null
    ...
    order by (power(radians(d.current_lat-trip.pickup_lat),2)+power(radians(d.current_lng-trip.pickup_lng)*cos(radians(trip.pickup_lat)),2)),d.id
    limit 1 for update;
